...
* **Scope & Delivery**:
  * Environment-aware configuration: dynamic Cloud Run `$PORT` binding (default 8000, overridable to 8080), environment classification properties (`is_production`, `is_staging`, `is_development`), and Cloud SQL Unix socket resolution.
  * Cloud SQL integration: PostgreSQL 16 managed database configuration with `settings.effective_database_url` appending Unix socket path.
  * Multi-stage production containerization: non-root execution (`eduvia:1001`), dynamic port binding, health check probing in `backend/Dockerfile`; Nginx SPA routing and dual-port listening in `frontend/Dockerfile` & `frontend/nginx.conf`.
  * Headless database migration runner: `backend/scripts/run_migrations.py` automating `alembic upgrade head` for pre-deployment pipelines.
  * Declarative Terraform IaC: `main.tf`, `cloud_run.tf`, `cloud_sql.tf`, `secret_manager.tf`, `variables.tf`, and `outputs.tf` declaring Cloud Run services, Cloud SQL PostgreSQL 16, Secret Manager secrets, IAM bindings, and Artifact Registry.
  * Continuous integration & deployment: `cloudbuild.yaml` orchestrating container builds, Artifact Registry pushes, migration jobs, and Cloud Run deployments.
  * Automated deployment testing: `backend/tests/test_cloud_deployment.py` with 8 targeted tests validating cloud configurations, container definitions, migration execution, and Terraform syntax.
* **Verified Milestone**: **217/217 backend tests passed** (8 cloud deployment tests + 209 baseline, 84% code coverage). Frontend production build clean (0 errors, 13/13 frontend unit & accessibility tests passing).
* **Gate Status**: **PHASE 12 — LOCKED**
* **Documentation**: [[05 - Development History/Phase 12 — Production Cloud Deployment & Staging|Phase 12 — Production Cloud Deployment & Staging]], [[05 - Development History/Reports/Phase 12 Report|Phase 12 Report]], [[Eduvia Production & Cloud Deployment|Eduvia Production & Cloud Deployment]]
...
| **Phase 12** | Production Cloud Deployment & Staging | 217 passed | Verified | **LOCKED** |
...
| `b19168a` | ahmedsamehzaky | 2026-09-19 | `feat(phase-12): complete production cloud deployment infrastructure` (**Phase 12 Locked**) |
...
### Operational Notes
* Phase 12 codified and validated the production infrastructure configuration as code (Terraform, Docker, Cloud Build). Live cloud provisioning on GCP billing accounts is executed on demand.
...
* Phase 12 Details: [[05 - Development History/Phase 12 — Production Cloud Deployment & Staging|Phase 12 — Production Cloud Deployment & Staging]] (Report: [[05 - Development History/Reports/Phase 12 Report|Phase 12 Report]])
